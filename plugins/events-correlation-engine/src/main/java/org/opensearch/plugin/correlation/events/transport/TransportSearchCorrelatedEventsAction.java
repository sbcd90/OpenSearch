                ...
                public void onResponse(SearchResponse response) {
                    ...
                    client.search(scoreSearchRequest, new ActionListener<>() {
                        ...
                        public void onResponse(SearchResponse response) {
                            ...
                            assert source != null;
                            long scoreTimestamp = (long) source.get("score_timestamp");
                            ...
                            client.search(searchRequest, new ActionListener<>() {
                                ...
                                public void onResponse(SearchResponse response) {
                                    ...
                                    if (response.getHits().getTotalHits().value != 1) {
                                        onFailures(new OpenSearchStatusException("Event not found in Correlation Index", RestStatus.INTERNAL_SERVER_ERROR));
                                    }
                                    ...
                                    CorrelationQueryBuilder correlationQueryBuilder = new CorrelationQueryBuilder("corr_vector", query, nearbyEvents, QueryBuilders.boolQuery()
                                        ...
                                        public void onResponse(SearchResponse response) {
                                            ...
                                            for (SearchHit hit: hits) {
                                                ...
                                                if (!source.get("event1").toString().equals(event)) {
                                                    String eventKey1 = source.get("event1").toString() + " " + source.get("index1").toString();

                                                    if (correlatedFindings.containsKey(eventKey1)) {
                                                        correlatedFindings.put(eventKey1, Math.max(correlatedFindings.get(eventKey1), hit.getScore()));
                                                    } else {
                                                        correlatedFindings.put(eventKey1, (double) hit.getScore());
                                                    }
                                                }
                                                if (!source.get("event2").toString().equals(event)) {
                                                    String eventKey2 = source.get("event2").toString() + " " + source.get("index2").toString();

                                                    if (correlatedFindings.containsKey(eventKey2)) {
                                                        correlatedFindings.put(eventKey2, Math.max(correlatedFindings.get(eventKey2), hit.getScore()));
                                                    } else {
                                            ...
                                            for (Map.Entry<String, Double> correlatedFinding: correlatedFindings.entrySet()) {
                                                ...
                                                eventWithScores.add(new EventWithScore(eventIndexSplit[1], eventIndexSplit[0], correlatedFinding.getValue(), List.of()));
                                            }
                                        }
