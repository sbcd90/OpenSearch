                                ...
                                correlationIndices.setupCorrelationIndex(setupTimestamp, new ActionListener<>() {
                                    ...
                                    public void onResponse(BulkResponse response) {
                                        if (!response.hasFailures()) {
                                            ...
                                            asyncAction.onFailures(new OpenSearchStatusException(response.toString(), RestStatus.INTERNAL_SERVER_ERROR));
                                        }
                                    }
    ...
    class AsyncStoreCorrelationAction {
        ...
        private void generateTimestampFeature() {
            ...
            BoolQueryBuilder queryBuilder = QueryBuilders.boolQuery()
                .mustNot(QueryBuilders.termQuery("score_timestamp", 0L));
            ...
            client.search(searchRequest, new ActionListener<>() {
                ...
                public void onResponse(SearchResponse response) {
                    ...
                    long scoreTimestamp = source != null? (long) source.get("score_timestamp"): 0L;
                    if (eventTimestamp - CorrelationIndices.FIXED_HISTORICAL_INTERVAL > scoreTimestamp) {
                        try {
                            Correlation scoreRootRecord = new Correlation(id,
                                NO_VERSION,
                                false,
                                0L,
                                "",
                                ...
                                public void onResponse(IndexResponse response) {
                                    if (response.status().equals(RestStatus.OK)) {
                                        if (request.getEventsAdjacencyList() == null || request.getEventsAdjacencyList().isEmpty()) {
                                            insertOrphanEvents(Long.valueOf(CorrelationIndices.FIXED_HISTORICAL_INTERVAL / 1000L).floatValue());
                                        } else {
                                            insertCorrelatedEvents(Long.valueOf(CorrelationIndices.FIXED_HISTORICAL_INTERVAL / 1000L).floatValue());
                                        }
                                    } else {
                                        onFailures(new OpenSearchStatusException("Failed to update Score Root record", RestStatus.INTERNAL_SERVER_ERROR));
                                    }
                                }
                    ...
                    try {
                        if (level == 0L) {
                            updateRootRecord(id, 50L, new ActionListener<>() {
                                ...
                                public void onResponse(IndexResponse response) {
                                    if (response.status().equals(RestStatus.OK)) {
                                        ...
                                        onFailures(new OpenSearchStatusException("Root Record not updated", RestStatus.INTERNAL_SERVER_ERROR));
                                    }
                                }
                            ...
                            if (eventTimestamp - timestamp > correlationTimeWindow) {
                                ...
                                CorrelationQueryBuilder correlationQueryBuilder = new CorrelationQueryBuilder("corr_vector", query, 3, QueryBuilders.boolQuery()
